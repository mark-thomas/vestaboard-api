import { characterCode, specialChar, emptyBoard, LINE_LENGTH, characterCodeArray, } from './values';
export function characterArrayFromString(string) {
...
// }
export function containsEscapeCharacter(input) {
    const test = /[*]/g;
...
// }
export function containsNonDisplayCharacter(input) {
    const test = /^(?:[A-Za-z0-9!@#$\(\)\-+&=;:'\"%,./?°\s]*)$/g;
...
//   return true;
// }
export function convertToCharCodeArray(string) {
...
export function isValidBoard(message) {
...
export function convertBoardLayoutToString(board) {
    let boardString = '';
...
export function createTransitionBoards(startingBoard, endingBoard) {
...
export function logTransitionBoards(transitionBoards) {
...
export function findDifferences(board1, board2) {
    const differences = [];
    // Loop through the board
